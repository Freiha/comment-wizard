function getPaginationHTML(comicId, numberOfPages, currentPage) {
  ...
  let u_cbox_paginate = document.getElementsByClassName(
    "u_cbox_paginate _paginateArea"
  )[0];
  ...
  if (currentPage > 1) {
    ...
    u_cbox_pre_a.setAttribute(
      "class",
      "u_cbox_pre u_cbox_pre_end N=a:myc.page,g:en_en"
    );
  ...
  for (let i = fromPage; i <= toPage; i++) {
    ...
    u_cbox_page.setAttribute(
      "class",
      "u_cbox_page N=a:myc.page,g:en_en first-child"
    );
    ...
    if (i === 0) u_cbox_page.classList.add("first-child");
    else if (i === numberOfPages) u_cbox_page.classList.add("last-child");
  ...
  if (currentPage < numberOfPages) {
    ...
    u_cbox_next_a.setAttribute(
      "class",
      "u_cbox_next u_cbox_next_end N=a:myc.page,g:en_en"
    );
...
function setUpPageWithPagination(comicId, pageNo, initialPageLoad) {
  ...
  if (allComments.length > 0) {
    ...
    const numberOfPages = Math.ceil(
      allComments.length / NUMBER_OF_ELEMENTS_IN_PAGE
    );

    const u_cbox_wrap = document.getElementsByClassName("u_cbox_wrap")[0];
    if (u_cbox_wrap)
      u_cbox_wrap.appendChild(
        getPaginationHTML(comicId, numberOfPages, pageNo)
      );

    const startIndex = NUMBER_OF_ELEMENTS_IN_PAGE * (pageNo - 1);
    let endIndex =
      NUMBER_OF_ELEMENTS_IN_PAGE + NUMBER_OF_ELEMENTS_IN_PAGE * (pageNo - 1);
    if (allComments.length < endIndex) endIndex = allComments.length;
    ...
    if (emptyListDiv) {
      ...
      dsc.innerText =
        "'" + loadedComicObject.title + "' does not have any comments yet.";
    } else {
      ...
      no_comments_yet_text.innerText =
        "'" + loadedComicObject.title + "' does not have any comments yet.";
