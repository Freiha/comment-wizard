function getReplies(u_cbox_comment, comic, episode, parentCommentNo, objectId) {
  const u_cbox_btn_reply = u_cbox_comment.getElementsByClassName(
  ...
  if (u_cbox_btn_reply.classList.contains("u_cbox_btn_reply_on")) {
    ...
    const replyBox = u_cbox_comment.getElementsByClassName(
      "u_cbox_reply_area _replyArea"
    )[0]; //childNodes.querySelector("[data-object-id='" + episode.id + "']");
    ...
    const reply_request_url_with_query = getURLWithQuery(
      reply_request_url,
      params
    );

    client.get(reply_request_url_with_query, function (response) {
...
function applyGetRepliesEventListener(
  u_cbox_btn_reply,
  u_cbox_comment,
  comic,
  episode,
