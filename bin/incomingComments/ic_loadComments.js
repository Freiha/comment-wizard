  ...
  if (
    ...
    window.processComments = function (data) {
      ...
      const completeCommentList = [
        ...data.result.bestList.map((comment) => ({
          ...comment,
          isTopComment: true,
        })),
      ...
      if (completeCommentList.length > 0) {
        ...
        const scriptElem = document.getElementById(
          "fetch_comments_script_" + objectId
        );
    ...
    for (const entry of Object.entries(comic.episodes)) {
      ...
      const params = {
        ...
        // 'sort': 'new',
        // 'useAltSort': 'true',
        token: switchOnFeature(KEY_SETTING_LIKE_DISLIKE_INDICATOR)
          ? window.getCookie("NEO_SES").replaceAll('"', "")
          : "",
        objectId: entry[0],
...
function getTitleEpisodeInfoAndExtractComments(
  objectIdsJsonList,
  objectId_url_dict,
  webcomic,
  dictOfComics
  ...
  client.get(
    "https://www.webtoons.com/comment/titleEpisodeInfo",
    function (response) {
      ...
      for (const [objectId, episodeInfo] of Object.entries(result)) {
        webcomic.addEpisode(
          ...
          objectId_url_dict[objectId],
          episodeInfo.episodeSeq
        );
      }
  ...
  client.get(webcomic.url, function (response) {
    ...
    client.get(latest_episode_url, function (response) {
      ...
      for (const item of episode_list) {
        ...
        objectId_url_dict[objectId] = item.children[0].href.replace(
          "/en/",
          `/${languageCode}/`
        );
      }
      getTitleEpisodeInfoAndExtractComments(
        objectIdsJsonList,
        objectId_url_dict,
        webcomic,
        dictOfComics
...
window.getComments = function () {
  ...
  if (commentArea) commentsWrap.removeChild(commentArea);
  const pagination = document.getElementsByClassName(
    "u_cbox_paginate _paginateArea"
  )[0];
  if (pagination) pagination.parentElement.removeChild(pagination);
};
