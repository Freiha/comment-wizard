...
const injectScripts = async function() {
    ...
    function addSettingsToPage(content) {
        ...
        (document.head || document.documentElement).appendChild(extensionSettingsDiv);
    }
    ...
    if (content.settings.incomingComments && document.location.href.match(new RegExp("https://www.webtoons.com/[a-z]{2,4}?/mycomment*"))) {
        const content_to_submit = {likeDislike: content.settings.likeDislike ? content.settings.likeDislike : false,
