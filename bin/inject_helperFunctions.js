...
function getLanguageFromURL() {
    ...
    {
        if (url.includes(baseurl + langCode + "/"))
            return langCode;
    }
    return "en";
}
...
const HttpClient = function () {
    this.get = function (aUrl, aCallback, action = "GET", body = null, header = null) {
...
function getDateString(date) {
    ...
    else if (getDays(diffSeconds) < 7)
        return (getDays(diffSeconds) + " days ago")

    return commentDate.getFullYear() + "-" + (commentDate.getMonth() + 1) + "-" + commentDate.getDate() + " " +
        commentDate.getHours() + ":" + commentDate.getMinutes();
}

function isRecrawlPossible() {
    return !LAST_CRAWL || getMinutes(Math.floor(new Date() - new Date(LAST_CRAWL))) > RECRAWL_INTERVAL;
}
