...
function addLinkToLoginDropdown(linkButton, incomingText, outgoingText, suffix = '#incoming') {
    ...
    linkButton.parentElement.insertBefore(link_listElement, linkButton.nextSibling);
        ...
        if (loginDropdown) {
            if (window.location.href.includes("/en/")) {
                ...
                if(commentLinkButton) {
                    addLinkToLoginDropdown(commentLinkButton, "Comments (IN)", "Comments (OUT)");
                }
            } else if (window.location.href.includes("/es/")) {
                const commentLinkButton = findLinkButton(loginDropdown, "Comentarios");
                if(commentLinkButton) {
                    addLinkToLoginDropdown(commentLinkButton, "Comentarios (IN)", "Comentarios (OUT)");
                }
            }
    }
}

if (!document.location.href.match(new RegExp("https://www.webtoons.com/[a-z]{2,4}?/mycomment*")))
