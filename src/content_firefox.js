...
function getBannerImageUrl() {
  return {
    adImage: getUrl("banner/AnD_banner.png"),
    backgroundImage: getUrl("banner/bg_banner.png"),
  };
}
...
const injectScripts = async function () {
  ...
  function addHeaderPathsToPage() {
    let bannerImageUrls = getBannerImageUrl();
    const bannerImageDiv = document.createElement("div");
    bannerImageDiv.setAttribute("id", "bannerImageDivUrl");
    bannerImageDiv.style.visibility = "hidden";
    bannerImageDiv.innerText = JSON.stringify({
      adImage: getUrl(bannerImageUrls.adImage),
      backgroundImage: getUrl(bannerImageUrls.backgroundImage),
    });
    (document.head || document.documentElement).appendChild(bannerImageDiv);
  }
